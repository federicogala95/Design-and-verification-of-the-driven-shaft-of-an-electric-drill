...
Jb=(pi*d_b^4)/32; %shaft section moment of inertia related to bearing diameter
Jg=(pi*d_g^4)/32;
Js=(pi*d_s^4)/32;
...
M_z_BK_slow=Rb_y*x(1:49);
V_y_BK_slow=Rb_y*ones(1,49);
M_z_KE_slow=Rb_y*x(50:137)+Fr*(x(50:137)-24);
V_y_KE_slow=-Re_y*ones(1,88);
M_z_slow=[M_z_BK_slow M_z_KE_slow];
V_y_slow=[V_y_BK_slow V_y_KE_slow];

M_y_BK_slow=Rb_z*x(1:49);
V_z_BK_slow=Rb_z*ones(1,49);
M_y_KE_slow=Rb_z*x(50:137)+Ft*(x(50:137)-24);
V_z_KE_slow=-Re_z*ones(1,88);
...
sigma_slow_BH=((M_y_slow(1:9).^2+M_z_slow(1:9).^2).^0.5)/(Ib)*(d_b/2);
sigma_slow_HC=((M_y_slow(10:64).^2+M_z_slow(10:64).^2).^0.5)/(Ig)*(d_g/2);
sigma_slow_CD=((M_y_slow(65:74).^2+M_z_slow(65:74).^2).^0.5)/(Is)*(d_s/2);
sigma_slow_DJ=((M_y_slow(75:129).^2+M_z_slow(75:129).^2).^0.5)/(Ig)*(d_g/2);
sigma_slow_JE=((M_y_slow(130:137).^2+M_z_slow(130:137).^2).^0.5)/(Ib)*(d_b/2);
sigma_slow=[sigma_slow_BH sigma_slow_HC sigma_slow_CD sigma_slow_DJ sigma_slow_JE];

tau_BH=T_slow(1:9)*d_b/(Jb*2);
tau_HK=T_slow(10:48)*d_g/(Jg*2);
tau_KE=zeros(1,89);
tau_slow=[tau_BH tau_HK tau_KE];

sigma_eq_slow=(sigma_slow.^2+3*tau_slow.^2).^.5; %Von Mises Criterium
...
Kt_b1=2.6;
Kt_t1=2.2;
SF_static=235/((Kt_b1*sigma_slow(9).^2+Kt_t1*tau_slow(9).^2).^.5)

plot(x,sigma_eq_slow,'k',x,sigma_slow,'b',x,tau_slow,'r');
legend('sigma eq','sigma','tau_T');
title('Shaft stress (SLOW T. GEARSET)');
...
sigma_eq_slow_fat1=((sigma_slow(9)*Kt_b1*q)^2+H*(Kt_t1*tau_slow(9)^2)).^.5;

sigma_eq_slow_fat1<(sigma_a_y)/SF;
ratio_stress_slow1=sigma_eq_slow_fat1/((sigma_a_y)/SF);

%2)GEAR-SHOULDER SITE (x=31.5):d=20 mm D=25 mm
sigma_eq_slow_fat2=((sigma_slow(64)*Kt_b2*q)^2+H*(Kt_t2*tau_slow(64)^2)).^.5;

sigma_eq_slow_fat2<(sigma_a_y)/SF;
ratio_stress_slow2=sigma_eq_slow_fat2/((sigma_a_y)/SF);

%3)SHUOLDER-GEAR SITE (x=36.5 mm):d=20 mm D=25 mm
sigma_eq_slow_fat3=((sigma_slow(75)*Kt_b2*q)^2+H*(Kt_t2*tau_slow(75)^2)).^.5;

sigma_eq_slow_fat3<(sigma_a_y)/SF;
ratio_stress_slow3=sigma_eq_slow_fat3/((sigma_a_y)/SF);

%4)GEAR-BEARING SITE (x=64 mm): d=15 mm D=20 mm
sigma_eq_slow_fat4=((sigma_slow(130)*Kt_b1*q)^2+H*(Kt_t1*tau_slow(130)^2)).^.5;

sigma_eq_slow_fat4<(sigma_a_y)/SF;
ratio_stress_slow4=sigma_eq_slow_fat4/((sigma_a_y)/SF);
...
ratio_stress_slow=[ratio_stress_slow1 ratio_stress_slow2 ratio_stress_slow3 ratio_stress_slow4].^(-1)
