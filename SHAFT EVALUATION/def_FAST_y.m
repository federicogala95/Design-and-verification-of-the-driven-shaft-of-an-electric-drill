...
y_AB=Re_y*x^3/(6*Ib*E_rig)+A*x+B;
y_BC=Re_y*x^3/(6*Ig*E_rig)+C*x+D;
y_CD=(Re_y+Fr)*x^3/(6*Ig*E_rig)-Fr*24*x^2/(2*E_rig*Ig)+E*x+F;
y_DE=(Re_y+Fr)*x^3/(6*Is*E_rig)-Fr*24*x^2/(2*E_rig*Is)+G*x+H;
y_EF=(Re_y+Fr)*x^3/(6*Ig*E_rig)-Fr*24*x^2/(2*E_rig*Ig)+J*x+L;
y_FG=(Re_y+Fr)*x^3/(6*Ib*E_rig)-Fr*24*x^2/(2*E_rig*Ib)+M*x+N;
...
%DEFLECTION ALONG Y:
y_AB=Re_y*x_AB.^3/(6*E_rig*Ib)+A*x_AB+B;
y_BC=Re_y*x_BC.^3/(6*E_rig*Ig)+C*x_BC+D;
y_CD=(Fr+Re_y)*x_CD.^3/(6*E_rig*Ig)-Fr*24*x_CD.^2/(2*E_rig*Ig)+E*x_CD+F;
y_DE=(Fr+Re_y)*x_DE.^3/(6*E_rig*Is)-Fr*24*x_DE.^2/(2*E_rig*Is)+G*x_DE+H;
y_EF=(Fr+Re_y)*x_EF.^3/(6*E_rig*Ig)-Fr*24*x_EF.^2/(2*E_rig*Ig)+J*x_EF+L;
y_FG=(Fr+Re_y)*x_FG.^3/(6*E_rig*Ib)-Fr*24*x_FG.^2/(2*E_rig*Ib)+M*x_FG+N;
%SLOPE ALONG Y:
y1_AB=Re_y*x_AB.^2/(2*E_rig*Ib)+A;
y1_BC=Re_y*x_BC.^2/(2*E_rig*Ig)+C;
y1_CD=(Fr+Re_y)*x_CD.^2/(2*E_rig*Ig)-Fr*24*x_CD/(E_rig*Ig)+E;
y1_DE=(Fr+Re_y)*x_DE.^2/(2*E_rig*Is)-Fr*24*x_DE/(E_rig*Is)+G;
y1_EF=(Fr+Re_y)*x_EF.^2/(2*E_rig*Ig)-Fr*24*x_EF/(E_rig*Ig)+J;
y1_FG=(Fr+Re_y)*x_FG.^2/(2*E_rig*Ib)-Fr*24*x_FG/(E_rig*Ib)+M;
%PLOT:
def_fast_y=flip([y_AB y_BC y_CD y_DE y_EF y_FG]);
slop_fast_y=flip([y1_AB y1_BC y1_CD y1_DE y1_EF y1_FG]);
figure(1);
plot(x,def_fast_y,'b');
figure(2);
plot(x,slop_fast_y,'r');
