%DEFLECTION EVALUATION for fast transmission along z:
...
z_AB=Re_z*x^3/(6*Ib*E_rig)+A*x+B;
z_BC=Re_z*x^3/(6*Ig*E_rig)+C*x+D;
z_CD=(Re_z+Ft)*x^3/(6*Ig*E_rig)-Ft*24*x^2/(2*E_rig*Ig)+E*x+F;
z_DE=(Re_z+Ft)*x^3/(6*Is*E_rig)-Ft*24*x^2/(2*E_rig*Is)+G*x+H;
z_EF=(Re_z+Ft)*x^3/(6*Ig*E_rig)-Ft*24*x^2/(2*E_rig*Ig)+J*x+L;
z_FG=(Re_z+Ft)*x^3/(6*Ib*E_rig)-Ft*24*x^2/(2*E_rig*Ib)+M*x+N;
...
%DEFLECTION ALONG Z:
z_AB=Re_z*x_AB.^3/(6*E_rig*Ib)+A*x_AB+B;
z_BC=Re_z*x_BC.^3/(6*E_rig*Ig)+C*x_BC+D;
z_CD=(Ft+Re_z)*x_CD.^3/(6*E_rig*Ig)-Ft*24*x_CD.^2/(2*E_rig*Ig)+E*x_CD+F;
z_DE=(Ft+Re_z)*x_DE.^3/(6*E_rig*Is)-Ft*24*x_DE.^2/(2*E_rig*Is)+G*x_DE+H;
z_EF=(Ft+Re_z)*x_EF.^3/(6*E_rig*Ig)-Ft*24*x_EF.^2/(2*E_rig*Ig)+J*x_EF+L;
z_FG=(Ft+Re_z)*x_FG.^3/(6*E_rig*Ib)-Ft*24*x_FG.^2/(2*E_rig*Ib)+M*x_FG+N;
%SLOPE ALONG Z:
z1_AB=Re_z*x_AB.^2/(2*E_rig*Ib)+A;
z1_BC=Re_z*x_BC.^2/(2*E_rig*Ig)+C;
z1_CD=(Ft+Re_z)*x_CD.^2/(2*E_rig*Ig)-Ft*24*x_CD/(E_rig*Ig)+E;
z1_DE=(Ft+Re_z)*x_DE.^2/(2*E_rig*Is)-Ft*24*x_DE/(E_rig*Is)+G;
z1_EF=(Ft+Re_z)*x_EF.^2/(2*E_rig*Ig)-Ft*24*x_EF/(E_rig*Ig)+J;
z1_FG=(Ft+Re_z)*x_FG.^2/(2*E_rig*Ib)-Ft*24*x_FG/(E_rig*Ib)+M;
%PLOT:
def_fast_z=flip([z_AB z_BC z_CD z_DE z_EF z_FG]);
slop_fast_z=flip([z1_AB z1_BC z1_CD z1_DE z1_EF z1_FG]);
figure(1);
plot(x,def_fast_z,'b');
figure(2);
plot(x,slop_fast_z,'r');
...
G_tors=79*10^3;
tors_ang_fast=T*6/(G_tors*Jb)+27.5*T/(G_tors*Jg)+5*T/(G_tors*Js)+7.5*T/(G_tors*Jg)
tors_ang_fast/46<0.044/1000